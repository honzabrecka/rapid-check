const { rng, sample, forAll } = require('../check')
...
describe('sample', () => {
  ...
  it('gen.constantly', () => {
    ...
    samples.forEach(value => expect(value).toBe(foo))
  })

  it('gen.choose', () => {
    const samples = sample(choose(1, 3))
    const s = new Set([1, 2, 3])
    samples.forEach(value => expect(s.has(value)).toBe(true))
  })

  it('gen.int', () => {
    const samples = sample(int)
    samples.forEach(value => expect(Number.isInteger(value)).toBe(true))
  })
  ...
  it('gen.bool', () => {
    ...
    samples.forEach(value => expect(bools.has(value)).toBe(true))
  })
})
...
describe('forAll shrinking', () => {
  it('gen.constantly shrinking', async () => {
    ...
    const {
      success,
      shrink: { min },
    } = await forAll(constantly(foo), prop)
  ...
  it('gen.tuple(gen.uint, gen.uint) shrinking', async () => {
    ...
    const {
      success,
      shrink: { min },
    } = await forAll(tuple(uint, uint), prop)
    expect(success).toBe(false)
  ...
  it('gen.choose(gen.tuple(gen.fmap(gen.uint)) shrinking', async () => {
    ...
    const {
      success,
      shrink: { min },
    } = await forAll(tuple(negative), prop)
    expect(success).toBe(false)
    expect(min).toEqual([-29])
  })
  ...
  it('gen.fmap shrinking', async () => {
    ...
    const {
      success,
      shrink: { min },
    } = await forAll(negative, prop, { count: 2000000 })
    expect(success).toBe(false)
  ...
  it('gen.oneOf shrinking', async () => {
    ...
    const {
      success,
      shrink: { min },
    } = await forAll(oneOf(negative, negative, uint), prop)
    expect(success).toBe(false)
    expect(min).toBe(29)
  })
